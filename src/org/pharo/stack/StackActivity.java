...
public class StackActivity extends Activity implements TextToSpeech.OnInitListener {
	StackVM vm;
	StackView view;
	StackImgList imgl;
	TextToSpeech mTts;
    ...
    boolean loadFromAssets(final StackActivity ctx) {
	...
	for(int i = 0; i < zips.length; i++) {
	    ...
	    while ((ze = zin.getNextEntry()) != null) {
	        Log.v("Stack Assets", "Unzipping " + ze.getName());
		String dest = imgdir + "/" + ze.getName();
                if(ze.isDirectory()) {
		    Log.v("Stack Assets", "Creating directory " + dest);
		    new File(dest).mkdirs();
		} else {
		    File df = new File(dest);
		    Log.v("Stack Assets", "Writing to " + dest);
